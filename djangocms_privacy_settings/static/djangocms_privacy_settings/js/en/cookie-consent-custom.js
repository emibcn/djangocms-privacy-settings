	...
	cookieconsent: function(options){
		...
		var settings_dialog = $(
			'<div class="ui-widget ui-corner-all cookieconsent-dialog-content cookieconsent-edit-settings">' +
				...
				'<div class="cookieconsent-level-changer-wrap">' +
					'<p style="text-align:center; clear:both; padding:5px 10px;" class="ui-state-highlight">\n' + 'Use the buttons below to see the different types of cookies you can choose.</p>' +
					'<div class="cookieconsent-level-changer" />' +
					'<div class="cookieconsent-permission-panels">' +
						'<div class="ui-corner-all cookieconsent-permission-panel-disallowed"><div class="ui-corner-top ui-widget-header cookieconsent-permission-panel-title red"><i class="fa fa-thumbs-down" aria-hidden="true"></i> Cookies deactived :</div><ul class="ui-widget ui-widget-content ui-corner-bottom" /></div>' +
						'<div class="ui-corner-all cookieconsent-permission-panel-allowed"><div class="ui-corner-top ui-widget-header cookieconsent-permission-panel-title green"><i class="fa fa-thumbs-up" aria-hidden="true"></i> Cookies actived :</div><ul class="ui-widget ui-widget-content ui-corner-bottom" /></div>' +
					'</div>' +
				'</div>' +
				(config.cookie_policy_url.length > 0?'<a href="'+config.cookie_policy_url+'" class="cookieconsent-cookie-policy-link btn-primary btn text-center">More</a>':'') +
			'</div>'
		);
		...
		if(config.edit_settings_element != null){
			switch(config.edit_settings_element[0].nodeName.toLowerCase()){
				case 'input':
					if(trim(config.edit_settings_element.val()).length == 0){ config.edit_settings_element.val('Change cookies settings'); }
					break;
				default:
					if(trim(config.edit_settings_element.html()).length == 0){ config.edit_settings_element.html('Change cookies settings'); }
			}
				...
				if(config._demo_mode || config.levels.saved() == null){
					var popup = $(
						'<div class="ui-widget ui-corner-top cookieconsent-popup">' +
							...
							(config.cookie_policy_url.length > 0?'<a href="'+config.cookie_policy_url+'" class="cookieconsent-cookie-policy-link btn btn-primary">More</a>':'') +
